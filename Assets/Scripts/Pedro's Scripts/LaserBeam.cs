...
public class LaserBeam : MonoBehaviour
{
    ...
    void CheckHit(RaycastHit hitInfo, Vector3 direction, LineRenderer laser)
    {
        ...
        else if (hitInfo.collider.gameObject.tag == "Refract")
        {
            ...
            // Check the name of the object to determine the refractive index
            switch (hitInfo.collider.gameObject.name)
            {
                case "Glass":
                    n2 = refractiveMaterials["Glass"];
                    break;
                case "Oil":
                    n2 = refractiveMaterials["Oil"];
                    break;
                case "Water":
                    n2 = refractiveMaterials["Water"];
                    break;
                default:
                    break;
            }
            ...
            Vector3 newRayStartPos = ray1.GetPoint(0.5f);
            Ray ray2 = new Ray(newRayStartPos, -refractedVector);

            RaycastHit hit2;
            if (Physics.Raycast(ray2, out hit2, 0.5f, 1))
            {
